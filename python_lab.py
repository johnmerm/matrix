...
## Task 15
exclude_zero_list = [ (i,j,k) for i in S for j in S for k in S if sum([i,j,k])==0 and not (i==0 and j==0 and k==0)]

## Task 16
first_of_tuples_list = [ (i,j,k) for i in S for j in S for k in S if sum([i,j,k])==0 and not (i==0 and j==0 and k==0)][0]

## Task 17
L1 = [1,2,3,1,2] # <-- want len(L1) != len(list(set(L1)))
...
## Task 23
square_dict =  {k:k**2 for k in list(range(0,100))}

## Task 24
D = {'red','white','blue'}
#identity_dict =  { x[0]:x[1] for x in set(zip(D,set(range(0,len(D))))) }
...
listdict2dict =  {x[0]:x[1] for x in list(zip(names,list(d.values())))}
...
## Task 30 
def list2dict(L, keylist): return { x[0]:x[1] for x in list(zip(keylist,L)) } 
